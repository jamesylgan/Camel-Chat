...
let st1 = create_pub_chat st0 1 "chatroom 1"
...
let tests_server = [
  "test init_state (skips state test)" >:: (fun _ -> assert_equal st0 ({
      state = st0.state;
      uid = 0;
      chatid = 0;
      response = None;
  ...
  "test create pub chat" >:: (fun _ -> assert_equal st1 ({
      st0 with response = (Some {userid = 1; cmd = "e";
                                 success = true; info = String ("chatroom 1");
                                 chatid = 1});
               state = ({st0' with pub_chat_list = [(1, [])]; pub_chat_names = [("chatroom 1", 1)]; chat_msg = [(1, [])]});
               chatid = 1
}));
